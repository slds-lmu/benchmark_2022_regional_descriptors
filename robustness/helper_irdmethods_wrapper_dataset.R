...
maire_wrapper = function(data, job, instance, ...) {
  ...
  filenam = paste(job$prob.name, arg_list$model_name, arg_list$id_x_interest, sep = "_")
  obsdata = readRDS(file.path("dataeval/prod", paste0("subsample_", arg_list$datastrategy),
                              paste0(paste(filenam, arg_list$subsamples, "", sep = "_"), ".rds")))
  box_largest = readRDS(file.path("dataeval/prod/largest_box", paste0(filenam, ".rds")))
